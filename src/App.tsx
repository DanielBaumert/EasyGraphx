import { Component, For, children, onMount } from 'solid-js';
import { ContextMenu, ContextOpenMode, Label, NavItem } from './api/UI';
import { gridStore, internalStore, setStore, setUserViewStore, setViewStore, store, userViewStore, viewStore} from './api/Store';
import {  setContextMenuOpen, locationContextMenu, isContextMenuOpen, selectedClass, setSelectedClass, setSelectedClassIndex } from './api/Signals';
import { onMouseDown, onMouseMove, onMouseUp } from './api/Peripheral/Mouse';
import { UMLClassComponent } from './api/UML/UMLClass';
import { Point } from './api/Drawing';
import { SvgView, UMLClassView } from './api/SVG';
import { UMLClass } from './api/UMLv2';
...
const App: Component = () => {
  ...
  function getPlacementLocation(): Point {
    const gridSnap = (gridStore.space ) / (gridStore.subCount + 1);
    ...
    x = ((x - viewStore.offset.x)) - (x % gridSnap);
    y = ((y - viewStore.offset.y)) - (y % gridSnap);
  ...
  function onContextMenuAddClass() {
    ...
    setUserViewStore("classes", [... userViewStore.classes, {
  ...
  return (
    <div
      ...
      <SvgView onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp}>
        <For each={userViewStore.classes}>
          {(umlClass, index) => <UMLClassView {...umlClass} onClick={() => {
              console.log("click");
      ...
      <div class='z-20 absolute bottom-4 left-4 flex flex-col'>
        <Label title={`x: ${viewStore.offset.x}`} />
        <Label title={`y: ${viewStore.offset.y}`} />
      </div>
    </div>
  );
};
